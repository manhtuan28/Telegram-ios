...
func infoItems(
    ...
    if case let .user(user) = data.peer {
        ...
        let userIdText = "\(user.id.id._value)"
        items[currentPeerInfoSection]!.append(
            PeerInfoScreenLabeledValueItem(
                id: AnyHashable("tuangram_user_id"),
                label: "ID",
                text: userIdText,
                textColor: .primary,
        ...
        if let _ = data.threadData {
            ...
            let channelIdText = "\(channel.id.id._value)"
            items[currentPeerInfoSection]!.append(
                PeerInfoScreenLabeledValueItem(
                    id: AnyHashable("tuangram_channel_id"),
                    label: "ID",
                    text: channelIdText,
                    textColor: .primary,
                    action: { node, progress in
                        UIPasteboard.general.string = channelIdText
                    }, requestLayout: { animated in
                        interaction.requestLayout(animated)
                    }
                )
